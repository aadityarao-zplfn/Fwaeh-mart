...
const CustomerOrderHistory = () => {
  ...
  const fetchOrders = async () => {
    try {
      // SIMPLIFIED query - remove complex joins for now
          ...
          const itemsWithProducts = await Promise.all(
            (items || []).map(async (item) => {
              ...
              if (productError) {
                ...
                return { ...item, products: { name: 'Unknown Product', image_url: '' } };
              }

              return { ...item, products: product || { name: 'Unknown Product', image_url: '' } };
            })
          );
